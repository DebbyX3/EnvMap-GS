...
# --------- NEW CODE
def read_images_from_dir(images_dir, process_length=-1):
    """
    Reads a sequence of images from a directory, sorted by filename.
    Returns: frames (np.ndarray), fps (default 30), image_names (list of file paths)
    """
    # Accept common image extensions
    exts = ('.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.webp')
    image_names = [os.path.join(images_dir, f) for f in sorted(os.listdir(images_dir)) if f.lower().endswith(exts)]
    if process_length > 0:
        image_names = image_names[:process_length]
    frames = []
...
def save_video(frames, output_video_path, fps=10, is_depths=False, grayscale=False):
    ...
    if is_depths:
        ...
        #d_min, d_max = frames.min(), frames.max() #original
        #new deb
        d_min = min(np.min(f) for f in frames)
        d_max = max(np.max(f) for f in frames)
        ''' #original
