...
if save_log_txt is None:
    save_log_txt = f'{dataset_base_folder}/dataset_info_donut_depth.txt'
...
if total_validated_points == 0:
    ...
    infinity_colors = np.array([np.mean([c for c, d in point_votes_infinity[idx]], axis=0) for idx in final_infinity_indices]).astype(np.uint8)
    object_colors = np.array([np.mean([c for c, d in point_votes_object[idx]], axis=0) for idx in final_object_indices]).astype(np.uint8)

    # --- Nuovo: raccogli le depth per i punti oggetto ---
    object_depths = np.array([
        np.mean([d for c, d in point_votes_object[idx]]) if len(point_votes_object[idx]) > 0 else object_threshold
        for idx in final_object_indices
    ])

    # 3. Posiziona i punti del cielo (già filtrati) ESATTAMENTE sulla sfera
    infinity_points = ico_points_pos[final_infinity_indices]

    # 4. Prendi i punti degli oggetti (già filtrati) e SPOSTALI SECONDO LA LORO DEPTH
    object_points_original = ico_points_pos[final_object_indices]
    vectors = object_points_original - center_of_scene
    norms = np.linalg.norm(vectors, axis=1, keepdims=True)
    unit_vectors = vectors / norms

    # Calcola il raggio scalato per ogni punto oggetto
    scaled_object_radii = sphere_radius * (object_depths / infinity_threshold)
    object_points_shifted = center_of_scene + unit_vectors * scaled_object_radii[:, None]
...
if save_ply_path is None:
    save_ply_path = f'{dataset_base_folder}/points3D_{subdivisions}subd-{round(sphere_radius, 4)}radius-donut_between_{object_threshold}_and_{infinity_threshold}_using_distance.ply'
